      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6 lg:px-8">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {trustItems.slice(0, 6).map((item, i) => (
            <div key={i} className="flex items-start gap-4 rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-6">
              <div className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#15803D] text-xs text-white">
                <Check className="h-3.5 w-3.5" />
              </div>
              <div>
                <h3 className="mb-1 text-sm font-semibold text-[#0F172A]">{item.label}</h3>
                <p className="text-sm text-[#475569] leading-relaxed">{item.value}</p>
              </div>
            </div>
          ))}
        </div>

        <div className="mt-12 rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-8">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                <div className={`mt-1.5 h-2 w-2 rounded-full ${i === 0 ? "bg-[#0F172A]" : i === 1 ? "bg-[#1D4ED8]" : "bg-[#15803D]"}`} />
                <div>
                  <h4 className="font-semibold text-[#0F172A]">{loc.city}</h4>
                  <p className="text-sm text-[#475569]">{loc.detail}</p>
                  <p className="text-sm text-[#475569]">{loc.phone}</p>
                </div>
              </div>
            ))}
          </div>
